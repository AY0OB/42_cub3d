...
{
	fd = open("textures/sprite_one.xpm", O_RDONLY);
	if (fd < 0)
		cube_abort(game, "Error: invalid path\n");
	close(fd);
	fd = open("textures/sprite_two.xpm", O_RDONLY);
	if (fd < 0)
		cube_abort(game, "Error: invalid path\n");
	close(fd);
	fd = open("textures/sprite_three.xpm", O_RDONLY);
	if (fd < 0)
		cube_abort(game, "Error: invalid path\n");
	close(fd);
	fd = open("textures/sprite_four.xpm", O_RDONLY);
	if (fd < 0)
		cube_abort(game, "Error: invalid path\n");
	close(fd);
	fd = open("textures/stick.xpm", O_RDONLY);
	if (fd < 0)
		cube_abort(game, "Error: invalid path\n");
	close(fd);
	fd = open("textures/sword.xpm", O_RDONLY);
	if (fd < 0)
		cube_abort(game, "Error: invalid path\n");
	close(fd);
}
