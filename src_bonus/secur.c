...
/*   Updated: 2025/12/01 17:00:45 by amairia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mlx_xpm_file_to_img_sec(t_game *game, t_texture *tex, char *path, int i)
{
	...
	if (!(tex->img))
	{
		...
		if (i == 0)
			...
			ft_printf_fd(2, "door <%s> ", path);
		else
			ft_printf_fd(2, "sprite <%s> ", path);
...
int	mlx_get_data_addr_sec(t_texture *tex, char *path, int i)
{
	...
	if (!(tex->data))
	{
		...
		if (i == 0)
			...
			ft_printf_fd(2, "door img <%s> ", path);
		else
			ft_printf_fd(2, "sprite img <%s>", path);
		ft_printf_fd(2, "cant be charged\n");
		return (-1);
	}
	return (0);
