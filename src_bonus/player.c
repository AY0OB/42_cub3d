...
/*   Updated: 2025/12/01 21:32:49 by ledupont         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	move_axis(t_player *p, t_game *game, float moveX, float moveY)
{
	...
	if (newmap_x >= 0
		&& game->map[(int)p->pos_y][newmap_x] != '1'
		&& game->map[(int)p->pos_y][newmap_x] != 'D')
		p->pos_x += moveX;
	if (newmap_y >= 0
		&& game->map[newmap_y][(int)p->pos_x] != '1'
		&& game->map[newmap_y][(int)p->pos_x] != 'D')
		p->pos_y += moveY;
}
